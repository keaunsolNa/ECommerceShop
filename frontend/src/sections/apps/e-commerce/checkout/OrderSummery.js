...
const OrderSummary = ({ checkout, show }) => (
  <Stack spacing={3}>
    <MainCard content={false}
              sx={{ borderRadius: show ? '4px' : '0 0 4px 4px', borderTop: show ? '1px inherit' : 'none' }}>
      <TableContainer>
          ...
          <TableBody>
            ...
            <TableRow>
              ...
              <TableCell align='right' sx={{ borderBottom: 'none' }}>
                {checkout.subtotal &&
                  <Typography variant='subtitle1'>{currency(checkout.subtotal).format()}</Typography>}
              </TableCell>
            </TableRow>
            <TableRow>
              ...
              <TableCell align='right' sx={{ borderBottom: 'none' }}>
                {checkout.shipping && (
                  <Typography
                    variant='subtitle1'>{checkout.shipping <= 0 ? '-' : currency(checkout.shipping).format()}</Typography>
                )}
              </TableCell>
            </TableRow>
            <TableRow>
