...
const EmployeeDetailModal = ({ selectedData, handleReload, handleOpen }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      if (!dupCheck) return;
      const response1 = isInsert ? axios.post('/memberBase', data) : axios.patch(`/memberBase`, data);
  ...
  return (
    <ScrollX>
      <MainCard
        title={isInsert ? '회원 카드 등록' : '회원 카드 수정'}
        secondary={
              ...
              <Grid container spacing={2.5}>
                ...
                <Grid item xs={12}>
                  <Stack spacing={1.25}>
                    ...
                    <CheckBox value={formik.values.agreeTou} {...getFieldProps('agreeTou')} checked={formik.values.agreeTou} />
                  </Stack>
                </Grid>
                <Grid item xs={12}>
                  <Stack spacing={1.25}>
                    <InputLabel>개인정보수집 동의</InputLabel>
                    <CheckBox value={formik.values.agreePiu} {...getFieldProps('agreePiu')} checked={formik.values.agreePiu} />
                  </Stack>
                </Grid>
                <Grid item xs={12}>
                  <Stack spacing={1.25}>
                    <InputLabel>마케팅 수집 동의</InputLabel>
                    <CheckBox value={formik.values.agreeMcc} {...getFieldProps('agreeMcc')} checked={formik.values.agreeMcc} />
                  </Stack>
                </Grid>
